...
![Version](https://img.shields.io/badge/version-1.1.4-blue) ![License](https://img.shields.io/badge/license-MIT-green) ![Chrome/Edge](https://img.shields.io/badge/Browser-Chrome%20|%20Edge-brightgreen) [![LINUX DO](https://img.shields.io/badge/LINUX%20DO-社区交流支持-2ea44f)](https://linux.do)
...
- **推荐题解可自行选择**：抓取到多篇参考题解后，可以在笔记面板里自主选择本次想优先讲解的题解，学习灵茶山艾府等大神的优雅题解；
...
![复习功能演示](./assets/readme-gallery/review_demo.gif)
...
- **FSRS 算法动态推演**：底层接入专业的 **FSRS 间隔重复算法**来量化记忆衰减程度。系统会根据你的历史掌握情况动态推演复习间隔，生疏题目增加出现频次，熟练题目自动拉长周期，用最小的复习成本建立长期记忆。
- **复习看板直观呈现**：打开扩展面板即可在概览页看到当日的复习进度。力扣列表中到达复习期限的题目会自动高亮并置顶，当天的遗留任务一触即达，省去繁琐的翻找过程。
- **闭环更新熟练度**：对于已进入复习计划的题目，拓展的力扣题目列表侧边提供了快捷的记忆刷新🧠入口。完成重刷后直接在此处更新评分，形成**做题、笔记、遗忘、复习、再评分**的完整学习闭环。
...
- **私教定向答疑**：在深度学习题目中可直接使用仅答疑模式，聚焦你当前卡住的实现细节、张量维度与训练逻辑，避免被冗长模板信息干扰。
...
- 新域名首次保存时，浏览器会直接弹出原生权限确认窗口，请在弹窗中允许访问你填写的 API 服务域名。
- 扩展仅向你填写的 API 服务发送 API Key 与请求数据，不会上传到开发者服务器。
- 如果你在浏览器原生权限弹窗中拒绝，该配置不会保存。
- 已授权的同域名再次保存不会重复弹窗。
- 如果你拒绝授权，该配置不会保存，并提示：`未授予该 API 服务的访问权限，API 配置暂未保存。`
#### 3. 修改您复制和粘贴的数据
...
这是因为扩展需要在本地题目、笔记或复习状态发生变化后，等待约三分钟再合并上传到你已开启的备份方式。这个权限只用于自动备份调度，不会读取新的网页内容，也不会扩大 Google Drive 或 WebDAV 的访问范围。
...
- [x] **支持自定义 FSRS 算法参数（v1.1.3 已上线）**：允许用户根据自己的遗忘曲线调整 FSRS 算法的 `maximum_interval` 等参数，形成更个性化的复习节奏。
- [ ] **适配洛谷以及Codeforces**：面向国内算法竞赛和刷题用户，支持在洛谷题目页生成结构化笔记，尽量复用现有算法笔记链路，同时保持站点逻辑隔离，避免影响 LeetCode / CodeFun2000 现有功能。
- [ ] **笔记页主题切换**：优先只针对独立笔记本页面增加主题切换能力，保留当前 Claude 的风格作为默认主题，再逐步探索类似于Typora的主题切换功能。不计划一开始就做全平台主题同步，避免过早影响 popup、options、content scripts 和第三方站点页面。
- [ ] **增加深度学习复习功能**：在现有力扣间隔复习系统的基础上，让深度学习手撕代码进入长期复习队列。
- [ ] **融入内置大语言模型与知识库**：在现有直连 API 和本地笔记的基础上，探索更完整的 AI 学习工作流，例如基于历史笔记、题单、复习记录进行上下文检索，让 AI 不只是生成单篇笔记，而是能结合你的长期学习资料给出更贴合个人学习习惯的回答。
- [x] **Google Drive 完整备份（v1.1.3 同版本补丁）**：在保留坚果云 WebDAV 的基础上，新增 Google Drive 作为推荐备份方案。当前只新增 Google Drive，不同时扩展 OneDrive 等更多云盘选项，避免权限和审核说明膨胀。
- [ ] **更多云盘方案评估**：OneDrive 等其他方案暂不进入当前版本落地范围，后续如继续推进，会继续遵守用户主动授权、用户主动触发和最小权限范围。
- [ ] **加入面试复盘功能**：面向算法岗、开发岗的面试准备场景，支持记录面试问题、追问、回答表现、薄弱知识点和后续复习任务，打造完整的求职备战系统。
- [ ] **探索手机端复习提醒**：仿照[灵感来源](https://github.com/vduggg/codex-im)，探索通过飞书等轻量手机端应用查看每日复习状态，并接收复习提醒。目标是让用户不仅能在浏览器里刷题和记笔记，也能在手机端快速看到今天要复习哪些题目，形成更自然的每日学习提醒链路。
...
+ **涉及权限和站点适配时要格外谨慎**：不要无故扩大 `host_permissions`、`content_scripts.matches` 或新增远程访问范围。浏览器扩展的权限变化会影响用户信任和 浏览器拓展的审核成本。
...
### 🥇 推荐方案：Google Drive 完整备份

适合希望减少坚果云流量限制影响、或在国外网络环境下使用更稳定云盘的用户。开启前需要先填写你自己的 Google OAuth Client ID，之后扩展会把完整备份文件保存到你自己的 Google Drive 可见文件夹。

- 备份内容：题目记录、题单、笔记、复习状态、同步元数据。
- 数据流向：当前浏览器直接上传到你的 Google Drive，不经过开发者服务器。
- 保存位置：`我的云端硬盘 / CodeNote Helper Backups / code-note-helper-full-backup.json`。
- 触发方式：先按下方教程创建并填写 Client ID，再点击“登录并测试”；浏览器会打开 Google 登录与授权页面。之后你可以点击“立即备份到 Google Drive”或“从 Google Drive 恢复”。
- 自动同步：开启后会在本地数据发生变化后等待约三分钟合并备份；三分钟内多次新增、修改或删除只会合并上传一次。如果授权失效，设置页会提示你重新登录。

<a id="google-drive-custom-oauth-client-id" name="google-drive-custom-oauth-client-id"></a>

### Google Drive 自定义 OAuth Client ID 配置教程

这个功能用于把 CodeNote Helper 的完整备份保存到你自己的 Google Drive。**Client ID** 类似坚果云 WebDAV 的应用密码配置，用于让 Google 确认你授权的是自己的备份应用。**备份数据不会经过开发者服务器**。

配置步骤：

1. 打开 [Google Cloud Console](https://console.cloud.google.com/)。
2. 创建或选择一个项目，例如 `CodeNote Helper Backup`。
3. 在项目中启用 **Google Drive API**。
4. 点击 **“创建凭证”**，选择 **“用户数据”**，然后点击“下一步”。
5. 在跳转到的 **OAuth 权限请求页面** 中，应用名称填写 `CodeNote Helper`；**用户支持邮箱选择你的 Google 邮箱**；应用徽标可以先不上传；**开发者联系信息 / 电子邮件地址填写你的邮箱**。
6. 授权范围使用 `https://www.googleapis.com/auth/drive.file`，用于让扩展创建和维护自己的备份文件夹和备份 JSON；**不要使用** `https://www.googleapis.com/auth/drive`，它是更大的整个 Google Drive 权限。
7. 创建 OAuth Client ID 时，应用类型选择 **“Web 应用”**。名称可以填写 `CodeNote Helper Google Drive Backup`。如果页面里有“已获授权的 JavaScript 来源”，可以先不填；重点是 **“已获授权的重定向 URI”**，这里填写 `https://nfcpikidobapnnnahapgpokkjnidemij.chromiumapp.org/google-drive`，然后点击创建。
8. 创建成功后，复制生成的 **Client ID**，通常长得像 `xxxxx.apps.googleusercontent.com`。这个就是后续要填到 CodeNote Helper options 页面里的 **Google OAuth Client ID**。
9. 进入“OAuth 权限请求页面”里的 **“目标对象”**，在 **“测试用户”** 处点击 **“Add users”**，添加你授权登录时要使用的 Google 邮箱。**插件登录授权时使用的邮箱要和这里添加的测试用户邮箱一致**。
10. 回到 CodeNote Helper 设置页，把 Client ID 粘贴到 Google Drive 区域并保存，先点击 **“登录并测试”**。**这一步只检查授权是否可用，不会覆盖已有备份数据**。
11. 如果这是已有数据的浏览器，确认授权可用后再点击 **“立即备份到 Google Drive”**。如果这是新设备或新浏览器，**不要先点“立即备份”**，请先点击 **“从 Google Drive 恢复”**，避免用空数据覆盖云端备份。
12. 备份成功后，你可以在 Google Drive 普通网页中看到 `CodeNote Helper Backups/code-note-helper-full-backup.json`。

### 🥈 保留方案：坚果云 WebDAV (完整备份)

坚果云入口继续保留给已有用户，会将所有数据（记录、题单、笔记）打包上传到你的坚果云空间。若你遇到上传流量限制、403 或国外网络访问不稳定，建议优先切换到 Google Drive。
...
- 如果你开启了 Google Drive 备份，数据只会同步到**你自己的** Google Drive，不经过开发者服务器。
