...
适合希望减少坚果云流量限制影响、或在国外网络环境下使用更稳定云盘的用户。正式版已内置 Google Drive 授权配置，你只需要在设置页点击“登录并测试”，授权后扩展会把完整备份文件保存到你自己的 Google Drive 可见文件夹。
...
- 触发方式：点击“登录并测试”后，浏览器会打开 Google 登录与授权页面。之后你可以点击“立即备份到 Google Drive”或“从 Google Drive 恢复”。
- 自动同步：开启后会在本地数据发生变化后等待约三分钟合并备份；三分钟内多次新增、修改或删除只会合并上传一次。如果授权失效，设置页会提示你重新登录。
- 浏览器差异：Chrome 使用浏览器内置授权能力。**Microsoft Edge浏览器无法采用Google Drive登录，后续会尝试OneDrive等登录方式**。

<a id="google-drive-custom-oauth-client-id" name="google-drive-custom-oauth-client-id"></a>

### Google Drive 高级 OAuth Client ID 选项

普通用户不需要自己创建 Google OAuth Client ID。正式版和当前 `dist/dev/` 构建都已内置对应授权配置，设置页中的 Client ID 输入框仅保留给未知扩展 ID 或特殊开发调试场景使用；输入框为空是正常状态。**备份数据不会经过开发者服务器**。

本地开发测试建议使用仓库脚本生成独立目录，不要手动修改根目录 `manifest.json`。当前本地 dev 构建默认写入 `dist/dev/manifest.json`，正式发布源 `manifest.json` 始终保留正式 Chrome Web Store OAuth Client ID：

```powershell
.\scripts\build-extension.ps1 -Target dev -Clean
```

生成后在 Chrome 或 Edge 的扩展管理页加载 `dist/dev/`。当前本地开发版应确认运行时 ID 为 `dphinnngjhhgmabdjeecajealghipkbj`；如果同一路径反复构建后 ID 发生变化，需要先更新 Google Cloud Console 中对应的测试授权配置，再更新仓库中的 dev 映射。这个本地测试配置已用于 Chrome 本地授权验证。正式发布包使用：

```powershell
.\scripts\build-extension.ps1 -Target prod -Clean
```

根目录 `manifest.json` 始终保留正式 Chrome Web Store 授权配置。正式打包请使用 `dist/prod/`，它只保留发布所需文件，避免把本地测试配置、文档和测试代码混入发布包。
