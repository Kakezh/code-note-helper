...
[“专门回应”总字数不少于 520 字；若用户原文为空，也要输出不少于 320 字的问题拆解与建议]
...
3. 必须输出“原题解评价”小节，评分与复杂度分析结构必须与完整笔记模式一致，禁止省略。
4. “💡 对用户疑问与体会的专门回应”是强制小节，必须逐条对应用户问题，禁止只写概述。
5. 若用户疑问包含“我的题解有什么问题”或同义表达，必须输出完整可运行代码块；只给文字视为失败。
6. 若需要改代码，必须同时输出“修改点清单”，逐条说明改动原因与收益。
7. “专门回应”总字数必须达标：非空疑问不少于 520 字；空疑问不少于 320 字。
...
1. **【默认保持参考逻辑】**
   - 默认情况下，需保持参考题解的核心算法与复杂度结论一致，不得无依据改写成另一套解法
   - 非翻译场景下，禁止随意重构代码结构或替换关键数据结构
   - 仅当原实现明显错误、无法通过题目或存在确定性缺陷时，才允许必要重写，并必须给出依据
...
4. **【允许的修改】**
   ...
   - ✅ 调整代码格式和缩进（保持逻辑不变）
   - ✅ 语言翻译【强制要求】：
    如果参考题解语言与【我的代码】语言不一致，必须翻译为【我的代码】语言。
    ...
    **不要优先保留原代码的表层写法。**
    翻译目标是：**用目标语言最自然、最主流、最符合该语言习惯的方式，表达同一算法。**
    禁止把“保留原逻辑”误解为“尽量逐句直译原代码”。

    ### 惯用实现优先规则（高优先级）
    在**不改变算法目标、核心思路、关键步骤职责、复杂度结论**的前提下：

    1. 默认优先使用【目标语言】的：
    - 官方标准库
    - 语言内建能力
    - 该语言社区中高频、主流、面试常见的惯用写法

    2. 若目标语言存在更自然、更简洁、更常见的标准实现，应**直接采用**，而不是保留原题解中的基础写法。

    3. 原题解中的以下表层写法，若目标语言中有等价且更惯用的表达，应优先替换：
    - 手写频率统计循环
    - 手动初始化计数容器
    - 手动队列/栈模拟
    - 手动堆维护细节
    ...
    ### 强制倾向
    若“保守直译写法”和“目标语言惯用写法”都正确，且：
    - 算法范式不变
    - 关键步骤职责不变
    - 复杂度结论不变

    **则必须优先选择目标语言惯用写法，不能因为参考题解使用了基础写法，就默认继续保留基础写法。**

    ### 何时不要做惯用化替换
    只有在以下情况，才保留原写法或更贴近原写法翻译：

    1. 该写法本身就是算法关键组成部分，替换后会弱化题型特征；
    2. 替换后会改变算法范式、关键步骤职责或复杂度结论；
    3. 原题解依赖某种容器的特定语义（如有序性、稳定性、随机访问特性、原地修改特性）；
    4. 用户明确要求：
    - “贴近原题解逐句翻译”
    - “保留基础写法教学”
    - “不要使用目标语言高级接口”
    ...
    ### 强制示例理解
    以下示例用于说明“优先保留算法本质，而非表层写法”的含义。满足“算法不变、复杂度不变、职责不变”时，应按这种倾向处理。

    #### 1）频率统计 / 哈希计数类
    - 若原题解是“手写哈希表计数循环”，而目标语言有更标准的计数接口/容器，则应优先使用目标语言标准计数能力。
    - 例如：
    - Java/C++ 手写 map 计数 -> Python 优先 \`Counter\`
    - Java 手写 \`map.put(x, map.getOrDefault(x, 0) + 1)\` -> Python 优先 \`Counter\` 或其他标准计数写法
    - Python \`Counter\` -> Java 可翻译为 \`HashMap + getOrDefault\` / \`merge\`
    - 若原题解依赖“有序映射”语义，则不能把 ordered map 直接替换成 unordered/hash map

    #### 2）队列 / BFS / 双端队列类
    - 若原题解本质是“队列职责”，则优先使用目标语言标准队列/双端队列，而不是列表头删、数组手动模拟。
    - 例如：
    - Python \`list.pop(0)\` 风格队列 -> 优先 \`collections.deque\`
    - Java \`LinkedList\` 仅作为普通队列使用 -> 优先 \`ArrayDeque\`
    - 其他语言若有更标准的 deque/queue 容器，也应优先采用

    #### 3）堆 / 优先队列类
    - 若原题解本质是“优先队列职责”，则优先使用目标语言标准堆/优先队列接口，而不是手动维护底层堆数组细节。
    - 例如：
    - Java \`PriorityQueue\` -> Python 优先 \`heapq\`
    - 手动维护 heap array 的参考实现 -> 若目标语言有标准 heap/priority queue，应优先改写为标准接口
    - C++ 需要优先队列职责时，优先 \`std::priority_queue\`
    - 但若题目本身就是在讲“如何手写堆”或“堆化过程”，则不要替换掉教学核心

    #### 4）排序 / 比较器类
    - 若原题解本质是“按某规则排序”，则优先使用目标语言中更自然的排序接口、key、comparator，而不是机械照搬原语言的比较器模板。
    - 例如：
    - Java comparator -> Python 优先 \`sort(key=...)\` / \`sorted(key=...)\`
    - C++ comparator lambda -> Java 可翻译为 \`Comparator\`
    - JavaScript 数值排序必须使用比较函数时，应写出目标语言惯用写法
    - 但若题解重点在“自定义比较规则如何设计”，则要保留比较逻辑的解释，不可只保留结果

    #### 5）字符串构造类
    - 若原题解频繁拼接字符串，翻译时应优先使用目标语言高频惯用的字符串构造方式。
    - 例如：
    - Java \`StringBuilder\` -> Python 常用 \`list + ''.join(...)\`
    - Python 多段字符串收集 -> Java 可翻译为 \`StringBuilder\`
    - JavaScript 可使用数组收集后 \`join('')\`
    - 不要为了贴近原代码表面形式而保留低频、冗长、不符合目标语言习惯的拼接方式

    #### 6）映射 / 集合类
    - 若原题解本质是“键值映射”或“去重集合”，则应优先使用目标语言标准 map/set 抽象，而不是用不自然的替代写法硬凑。
    - 例如：
    - JavaScript 中需要真正 map 语义时，优先 \`Map\`
    - 去重需求应优先标准 \`Set\` / \`set\`
    - 仅当题目明确依赖数组下标、固定字符集、位图等特性时，才保留数组/位运算实现

    ### 明确禁止
    - 禁止把“保留原逻辑”理解为“保留原代码表层写法”
    - 禁止因为参考题解使用了基础写法，就放弃目标语言更自然的标准实现
    - 禁止在复杂度不变的前提下，仍优先选择更冗长、更不符合目标语言习惯的写法
    - 禁止为了“看起来更高级”而替换成会改变算法本质的另一套方案

    ### 若发生翻译，必须额外输出【语言翻译说明】
    必须包含以下内容：

    1. 原始语言 -> 目标语言，以及触发翻译原因；
    2. 【保留项】说明保留了哪些算法思想、关键步骤职责、复杂度结论；
    3. 【惯用化优化项】说明哪些地方改用了目标语言更自然的标准接口/数据结构；
    4. 【替换理由】说明这些替换为什么不改变算法本质，只是更符合目标语言习惯；
    5. 【翻译对照说明】说明关键语法、标准库调用、数据结构职责如何与原题解等价映射，避免生硬直译。
    ...
    除非用户明确要求保守直译，否则不要因为参考题解写得基础，就主动压制目标语言更自然的表达。

5. **【违规示例】**
   - ❌ 在非翻译场景下，随意把 \`for i in range(n)\` 改成 \`for i, num in enumerate(nums)\`
   - ❌ 在非翻译场景下，随意把 \`if len(s) == 0\` 改成 \`if not s\`
